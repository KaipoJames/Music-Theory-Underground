...
// const myChord = Chord.getChord("min7", "A4", "C4");
// console.log(myChord);
// const myChord2 = Chord.getChord("min7", "Ab4", "Cb4");
// console.log(myChord2);
// const A_flat = Chord.get("Abmaj7");
// console.log(A_flat);

/* const keyChords = Key.majorKey("C");
console.log(keyChords.chords); */
var sChords = _tonal.Scale.scaleChords("C major");

console.log(sChords);
var noteCombs = [];
var finalComb = [];

var getCombinations = function getCombinations(arr, n, r, index, newArr, i) {
  if (index === r) {
    for (var _j = 0; _j < r; _j++) {
      noteCombs.push(newArr[_j]); //console.log(newArr[j] + " ");
    } //console.log("");
  ...
  newArr[index] = arr[i];
  getCombinations(arr, n, r, index + 1, newArr, i + 1);
  getCombinations(arr, n, r, index, newArr, i + 1);
};

var printCombinations = function printCombinations(arr, n, r) {
  var data = [];
  getCombinations(arr, n, r, 0, data, 0);
};
/* const myScale = Scale.scaleChords("C major");
console.log(myScale); */


var cScale = _tonal.Scale.get("C major");

var noteArr = cScale.notes; //console.log(noteArr);

var noOfNotes = 3;
var n = cScale.notes.length; //console.log(n);
//printCombinations(noteArr, n, noOfNotes);
...
for (i = 0, j = noteCombs.length; i < j; i += chunk) {
  temparray = noteCombs.slice(i, i + chunk); //console.log(temparray);

  var tmpArr = _tonal.Chord.detect(temparray); //console.log(tmpArr);


  if (tmpArr.length != 0) {
    finalComb.push(tmpArr);
  } // do whatever

} //console.log(noteCombs.length);
//console.log("second Array: ");


for (var _i = 0, _finalComb = finalComb; _i < _finalComb.length; _i++) {//console.log(c);

  var c = _finalComb[_i];
} //console.log(finalComb.length);
// // return a list of scales that a specified chord fits in
// const whatScales = Chord.chordScales("Ab7b9b13");
// for (var i = 0; i < whatScales.length; i++) {
//   console.log(whatScales[i]);
// }
...
var octaveNoteSelector = document.querySelector("#octave-select");
var notesDisplay = document.querySelector("#notes");
var intervalDisplay = document.querySelector("#intervals");
...
var app = {
  init: function init() {
    ...
    this.setUpOctaves();
    this.setUpButtons();
    this.setUpEventListeners(); // this.updateChordsNotesDisplay();
  },
  ...
  setUpOctaves: function setUpOctaves() {
    ...
    octaves.forEach(function (octaveName) {
      var octaveNameOption = _this2.createElement("option", octaveName);
  ...
  setUpButtons: function setUpButtons() {
    ...
    chords.forEach(function (chordSymbol) {
      var chordSymbolBtn = _this3.createElement("button", chordSymbol);

      buttons.appendChild(chordSymbolBtn);
    });
  },
  displayAndPlayChords: function displayAndPlayChords() {
    // Update Intervals
    var chordSymbol = _tonal.ChordType.get(event.target.innerHTML);

    var chordIntervals = chordSymbol.intervals;
    intervalDisplay.innerHTML = chordIntervals.join(" - ");
    var chordName = event.target.innerHTML;
    var currentNote = startNoteSelector.value;
    var currentOctave = octaveNoteSelector.value;
    var combined = currentNote + chordName;

    var chordObject = _tonal.Chord.get(combined);

    var chordNotes = chordObject.notes;
...
var soundEngine = {
  init: function init() {
    var noteLength = 2400;
    var timeIndex = 0; // Loop through the midi values of every note(@note/tonaljs)

    for (var _i2 = 24; _i2 <= 96; _i2++) {
      sound["_sprite"][_i2] = [timeIndex, noteLength];
  ...
  playChord: function playChord(noteSequence) {
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var noteName = _step.value;

        var midiInt = _tonal.Note.midi(noteName);
...
app.init();
},{"@tonaljs/tonal":"../node_modules/@tonaljs/tonal/dist/index.es.js","howler":"../node_modules/howler/dist/howler.js"}],"../../../../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
